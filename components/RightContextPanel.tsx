...
export const RightContextPanel: React.FC<RightContextPanelProps> = ({
  ...
  return (
    <aside className="w-80 bg-white border-l border-slate-300 shadow-xl flex flex-col justify-between select-none z-30 animate-in slide-in-from-right duration-200">
      {/* Header */}
      <div className="bg-[#1C2A45] text-white p-3.5 flex items-center justify-between border-b border-[#0A192F]">
        <div className="flex items-center space-x-2">
          <ShieldCheck className="w-4 h-4 text-emerald-400" />
          <h3 className="text-xs font-bold">사건 상세 정보</h3>
        </div>
        <button 
          onClick={onClose}
      ...
      <div className="p-4 overflow-y-auto space-y-5 text-xs">
        ...
        <div className="bg-[#F8FAFC] border border-[#D5DBE2] rounded p-3.5 space-y-2">
          ...
          <div className="space-y-1.5 pt-1 text-[11px]">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold font-mono text-[#0A60C2]">{metadata.stakeholdersMeetingDate}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between font-bold text-slate-900 border-b border-slate-200 pb-1.5">
            <span className="flex items-center gap-1.5">
              <Hash className="w-3.5 h-3.5 text-[#0A60C2]" /> 증거 무결성 검증 (Chain of Custody)
            </span>
          ...
          <div className="bg-[#1C2A45] text-slate-200 p-3 rounded font-mono text-[10px] space-y-1.5 border border-slate-700 shadow-inner">
            <div className="text-amber-300 font-bold flex items-center gap-1 font-sans">
              <Lock className="w-3 h-3 text-emerald-400" /> 최신 SHA-256 해시:
            </div>
            <div className="break-all text-slate-300 bg-[#0A192F] p-2 rounded border border-slate-800 leading-tight">
              {evidenceItems[0]?.sha256Hash || 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'}
            </div>
            <div className="flex justify-between text-[9px] text-slate-400 pt-1 font-sans">
              <span>입수: {evidenceItems[0]?.intakeTimestamp}</span>
              <span className="text-emerald-400">무결성 보증</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="font-bold text-slate-900 border-b border-slate-200 pb-1.5 flex items-center gap-1.5">
            <Scale className="w-3.5 h-3.5 text-[#0A60C2]" /> 재판부 & 관재인 정보
          </div>

          <div className="bg-[#F8FAFC] border border-[#D5DBE2] p-3 rounded space-y-2">
            <div>
            ...
            <div>
              ...
              <div className="font-bold text-[#0A60C2]">{metadata.administrator}</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="font-bold text-slate-900 border-b border-slate-200 pb-1.5 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
              <FileText className="w-3.5 h-3.5 text-[#0A60C2]" /> 증거 서류 목록 ({evidenceItems.length})
            </span>
          </div>

          <div className="space-y-1.5">
            {evidenceItems.slice(0, 4).map((ev) => (
              <div key={ev.id} className="bg-white border border-[#D5DBE2] p-2 rounded text-[11px] hover:border-[#0A60C2] transition-colors">
                <div className="flex items-center justify-between">
                  <span className="font-mono font-bold text-[#0A60C2]">{ev.courtLabel}</span>
                ...
                <div className="text-[9px] text-slate-500 font-mono truncate mt-0.5">해시: {ev.sha256Hash.substring(0, 16)}...</div>
              </div>
            ))}
          </div>
        </div>
